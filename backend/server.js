...
const notificationsQueueUrl = process.env.NOTIFICATIONS_SQS_QUEUE_URL || '';
...
app.get('/api/feiras', async (req, res) => {
...
app.post('/api/contato', async (req, res) => {
  ...
  if (queueUrl) {
    ...
    try {
      ...
      return res.status(500).json({ success: false, message: 'Mensagem salva, mas falha ao enviar para fila.' });
    }
  }
...
app.post('/api/notificacoes', async (req, res) => {
  ...
  const inscricaoData = new Date().toISOString();
  const notificationPayload = {
    nome,
    email,
    data_inscricao: inscricaoData
  };

  const connection = await pool.getConnection();
  try {
    const [result] = await connection.query(
      'INSERT INTO notificacoes (nome, email, data_inscricao) VALUES (?, ?, ?)',
      [nome, email, inscricaoData]
    );
    notificationPayload.inscricao_id = result.insertId;
  } finally {
    connection.release();
  }
  ...
  if (notificationsQueueUrl) {
    ...
    try {
      ...
      console.error('Erro ao enviar inscrição para SQS:', error);
      return res.status(500).json({ success: false, message: 'Inscrição salva, mas falha ao enviar para fila.' });
    }
  }
  ...
  if (snsTopicArn) {
    const params = {
      ...
      Message: `Nova inscrição para notificações: ${nome} <${email}>`,
      Subject: 'Nova inscrição de notificações'
    };

    try {
      await snsClient.send(new PublishCommand(params));
    } catch (error) {
      console.error('Erro ao publicar em SNS:', error);
      return res.status(500).json({ success: false, message: 'Inscrição salva, mas falha ao enviar notificação via SNS.' });
    }
  }

  res.json({ success: true, message: 'Inscrição para notificações cadastrada com sucesso!' });
});
